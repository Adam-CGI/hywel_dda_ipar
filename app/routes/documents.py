"""
Document upload routes for IPAR Document Miner.
"""
import logging
import json
...
def reindex_document(doc_id):
    ...
    try:
        ...
        # Step 2: Import chunking and indexing services
        from app.services.chunking_service import ChunkingService
        from app.services.indexing_pipeline_service import IndexingPipelineService
        
        chunking_service = ChunkingService()
        indexing_service = IndexingPipelineService()
        ...
        chunks = chunking_service.chunk_document(
            ...
            metadata={
                'logical_id': document.get('logical_id', doc_id),
                'version': document.get('version', 1),
                'source_uri': document.get('source_uri', ''),
                'title': document.get('origin_filename', ''),
                'origin_filename': document.get('origin_filename', '')
            }
        )
        ...
        result = indexing_service.index_chunks(chunks)
